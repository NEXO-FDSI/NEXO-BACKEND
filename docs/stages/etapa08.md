...
- Objetivo: integrar el LLM (Ollama local en dev, swap a API real por configuración) sobre el índice ATT&CK ya cargado. El LLM redacta un párrafo de análisis grounded a partir del texto real de las técnicas (recuperación por ID exacto en Chroma, no búsqueda semántica) — nunca decide qué técnicas van en el informe, eso ya lo resolvió la Etapa 6.
- Implementa: `app/ai_component/` (vectorstore, llm_client, prompt_builder, service), `scripts/seed_technique_embeddings.py`, integración con `app/reporting/`.
- Dependencias: Etapa 6 (correlación) y Etapa 7 (informe).
- Verificación: tests con LLM/vector store mockeados; prueba manual con Ollama real generando análisis grounded.
- Aceptación: el LLM nunca se invoca sin contexto grounded; un fallo del LLM nunca rompe el informe; el vector store queda detrás de una interfaz (`VectorStore`) para poder migrar a un backend cloud (ej. Pinecone) sin tocar la lógica de RAG.
- Estado: **prompt listo** (ver `etapa-08-ai-component-rag.md`).
